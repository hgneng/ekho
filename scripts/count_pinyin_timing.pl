...
my @initials = ('b', 'p', 'm', 'f',
   ...
   'z', 'c', 's',
   'w', 'y',);
my @finals = ('i', 'u', 'v', 'a', 'ia', 'ua', 'o', 'uo',
    'e', 'ie', 've', 'ai', 'uai', 'ei',
    'ao', 'iao', 'ou', 'an', 'ian', 'uan',
    'en', 'in', 'ang', 'iang', 'uang',
    'eng', 'ing', 'ong', 'iong');
our %final_timing;
our %initial_timing;
our %timing;
our @tone_factor;
my $different = $#initials + $#finals + 2;
...
# count tone factor
# take tone 1 as 1
  my @tone_total_percent = (1, 0, 0, 0, 0);
  my @tone_total_count = (1, 0, 0, 0, 0);
  foreach my $initial (@initials) {
    foreach my $final (@finals) {
...
sub count_timing {
  ...
  my $final_count = 0;
  foreach my $final (@finals) {
    $final_min = $final_timing{$final} if ($final_timing{$final} < $final_min && $final_timing{$final});
    $final_max = $final_timing{$final} if ($final_timing{$final} > $final_max);
    $final_total += $final_timing{$final};
    $final_count++ if ($final_timing{$final});
  }
  my $final_avg = $final_total / $final_count;
