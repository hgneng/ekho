...
int Ekho::speakPcm(short *pcm, int frames, void *arg,
...
int Ekho::writeToSonicStream(short *pcm, int frames,
    bool in_word_context, bool forbid_overlap) {
  if (!mSonicStream)
    return 0;
...
void Ekho::finishWritePcm(void) {
  writePcm(0, 0, this);
}

int Ekho::writePcm(short *pcm, int frames, void *arg,
    bool in_word_context, bool forbid_overlap) {
...
string Ekho::getVoice(void) {
    return this->mDict.getVoice();
}

int Ekho::speak(string text,
        void (*pCallback)(void*),
        void* pCallbackArgs) {
  this->initSound();
  this->isPaused = false;
...
int Ekho::stopAndSpeak(string text,
    void (*pCallback)(void*),
    void* pCallbackArgs) {
  this->stop();
